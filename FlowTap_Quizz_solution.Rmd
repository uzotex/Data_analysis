...
```
In the coefficients table the coefficient for the variable "Education_num" is NA, i.e. with a missing value,this is an indication that the covariate "educationnum" is collinear with some other predictors. Therefore, we have to exclude it from the list of predictor variables and fit the model again. 
...
remove column 24 from the design matrix, also weidentify which predictor corresponds to column 24
...
i.e. collinear.Therefore we remove the covariate "education_num" and fit a new model - "glm.model2
...
value of 1 is equivalent to an yearly income of more than 50K, and a value of 0 means an income of less than 50K.
...
percentage of correctly predicted response values:
